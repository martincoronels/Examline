    ...
    <div class="container">
      <form method="post" id="formulario">
        ...
        <div class="login-email">
          ...
          <p>&nbsp;</p>
			    <p class="login-register-text">¿Tenes una cuenta? <a href="02-loginprofes.php">Ingresa aca</a></p>
      </form>
...
if (isset($_POST['register'])) {//Si envia el register
    if (strlen($_POST['username']) >= 1 && strlen($_POST['email']) >= 1 && strlen($_POST['password']) >= 1) {
        ...
        $primerresultado = mysqli_query($conex, "SELECT * FROM tabla_profes");//Hago la consulta
        while ($consulta = mysqli_fetch_array($primerresultado)){
          ...
          //Recorro ese array  
          if ($consulta["emailprofe"] == $email){//Si el email es igual al ingresado
        ...
        elseif (strlen($_POST['password'])<8){// debe de validar que tenga mas de 8 caracteres lo ingresado
          echo '<script language="javascript">
            var formu = document.getElementById("formulario");
            var para = document.createElement("h3");
            para.classList.add("avisoEmail");
            ...
            $consulta = "INSERT INTO tabla_profes(nombreprofe, contraseñaprofe, emailprofe) VALUES ('$nombre', '$contra', '$email')";
