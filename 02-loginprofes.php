    ...
    <div class="container">
      ...
      <div class="login-email">
        ...
        <input type="submit" name="login" class="button">
			<p>&nbsp;</p>
			<p class="login-register-text">¿Todavia no te sumaste?  <a href="03-registroprofes.php">Registrate</a>.</p>
      <p class="login-register-text">¿Sos alumno?  <a href="10-loginalumnos.php">Entra aca</a>.</p>
      </form>
    </div>
	</div>
...
if (isset($_POST['login'])){//Cuando se envia el login
    if (strlen($_POST['email']) >= 1 && strlen($_POST['password']) >= 1){//Si el largo del email y el de la
      //contraseña son mayores o iguales a 1
        ...
        $query = "SELECT * FROM tabla_profes WHERE emailprofe = '$email' LIMIT 1";//Hago la consulta
        ...
        if($result){
            if($result && mysqli_num_rows($result)>0){//Si devuelve algo y tiene filas
                ...
                $idprofe = $user_data["idprofe"];//Tomo el id del profe
                if($user_data["contraseñaprofe"] === $contra){//Si la contraseña coincide
                    $_SESSION["nombreprofe"]=$user_data["nombreprofe"];
                    $_SESSION["idprofe"]=$user_data["idprofe"];
                    //Guardo la informacion en variables de sesion
                    header("location:04-paginaprofe.php?idprofe=$idprofe");//Redireccion a la pagina principal del profe
                    die;
                } else{
